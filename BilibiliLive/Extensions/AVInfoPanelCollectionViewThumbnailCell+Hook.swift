...
private extension UICollectionViewCell {
    static func swizzAVInfoPanelCollectionViewThumbnailCell() {
        guard let swizzledClass: AnyClass = NSClassFromString("AVInfoPanelCollectionViewThumbnailCell") else {
            Logger.warn("AVInfoPanelCollectionViewThumbnailCell class not found for swizzling")
            return
        }

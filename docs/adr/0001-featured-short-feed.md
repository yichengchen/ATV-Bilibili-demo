...
保留现有 `推荐` 网格页，并在其后新增独立的 `精选` Tab。`精选` 使用推荐接口过滤出短视频，采用“左列表右预览”的浏览态和独立的正式播放态。正式播放继续复用现有 DASH 播放链路和播放器插件体系，但通过新的 `VideoSequenceProvider`、`VideoPlayerMode` 和 `PlayContextCache` 支撑短视频流的上下切换、预热和失败恢复。
